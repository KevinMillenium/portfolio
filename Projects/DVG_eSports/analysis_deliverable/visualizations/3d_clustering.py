...
feature_columns = ["name", "Winrate", "Kills", "Deaths", "Assists"]
MAX_CLUSTERS = 15
...
def parse_args():
    ...
    parser.add_argument('-d', help='path to data file', default='../data/graph_data.csv')
...
def export_centroid_idx(centroids, idx, centroids_sklearn, idx_sklearn):
    ...
    with open(output_dir + "/clusters.json", 'w') as outfile:
...
def min_max_scale(data):
    ...
    min_col_values = np.min(data, axis=0)
    max_col_values = np.max(data, axis=0)

    return (data - min_col_values) / (max_col_values - min_col_values)
...
def cluster_songs(music_data, max_iters=300):
    ...
    for k in range(1,MAX_CLUSTERS):
        clusters.append(k)
        kmeans = Kmeans(data, k, max_iters)
        centroids, idx = kmeans.run()
        errors.append(kmeans.inertia(centroids, idx))
